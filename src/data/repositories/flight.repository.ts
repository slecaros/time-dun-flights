...
export const FlightRepository: FlightRepositoryInterface = {
  ...
  getByFlightCode: async (flightCode) => {
    const flight = await FlightModel.findOne({ flightCode: flightCode });
  ...
  updatePassenger: async ({search: {flightCode, passengerId}, update: {passenger}}) => {
    const search = { flightCode: flightCode, 'passengers.id': passengerId };
    const update = { $set: {
      ...(passenger.name && {'passengers.$.name': passenger.name}),
      ...(passenger.age && {'passengers.$.age': passenger.age}),
      ...(passenger.flightCategory && {'passengers.$.flightCategory': passenger.flightCategory}),
      ...(passenger.hasConnections && {'passengers.$.hasConnections': passenger.hasConnections}),
      ...(passenger.hasCheckedBaggage && {'passengers.$.hasCheckedBaggage': passenger.hasCheckedBaggage}),
    }};
  ...
  removePassenger: async (params) => {
    const flight = await FlightModel.findOneAndUpdate({ flightCode: params.flightCode }, { $pull: { passengers: { id: params.passengerId } } }, { new: true });
    return flight ? flight.toJSON<Flight>() : null;
  },

  deleteFlight: async (flightCode) => {
    const flight = await FlightModel.findOneAndDelete({ flightCode: flightCode });
    return flight ? flight.toJSON<Flight>() : null;
  }
}